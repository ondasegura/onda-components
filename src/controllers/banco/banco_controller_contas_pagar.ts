...
export const banco_controller_contas_pagar = class banco_controller_contas_pagar {
    static api = class api {
        static async criar(props: t.Banco.Controllers.ContaPagar.Criar.Input) {
            try {
                ...
                const response = await utils.api.servidor_backend.post(String(PUBLIC_BASE_URL_BACKEND), "/conta-pagar", props, false);
                ...
                store.setState((state) => { state.states.formulario.loading = false });
            }
        }
        ...
        static async buscar_pelo_filtro(props: t.Banco.Controllers.ContaPagar.BuscarPeloFiltro.Input): Promise<t.Banco.Controllers.ContaPagar.BuscarPeloFiltro.Output> {
            ...
            if (results?.data?.conta_pagar) {
                store.setState((state) => { state.states.pagina.conta_pagar = results; });
            }
        ...
        static async buscar_pelo_id(props: t.Banco.Controllers.ContaPagar.BuscarPeloId.Input): Promise<t.Banco.Controllers.ContaPagar.BuscarPeloId.Output> {
            ...
            if (results?.data?.conta_pagar) store.setState((state) => { state.states.modal.conta_pagar_item = results; })
        ...
        static async atualizar_pelo_id(props: t.Banco.Controllers.ContaPagar.AtualizarPeloId.Input): Promise<t.Banco.Controllers.ContaPagar.AtualizarPeloId.Output> {
            ...
            if (contaPagarAtualizada) {
                store.setState((state) => {
                    state.states.pagina.conta_pagar.data.conta_pagar = utils.update_context.update_array_itens({ oldArray: state.states.pagina.conta_pagar.data.conta_pagar, newItem: [contaPagarAtualizada], });
                    if (state.states.modal.conta_pagar_item.data.conta_pagar?._id === contaPagarAtualizada._id) { state.states.modal.conta_pagar_item = { data: { conta_pagar: contaPagarAtualizada } }; }
                });
            }
        ...
        static async deletar_pelo_id(props: t.Banco.Controllers.ContaPagar.DeletarPeloId.Input): Promise<t.Banco.Controllers.ContaPagar.DeletarPeloId.Output> {
            ...
            store.setState((state) => {
                state.states.pagina.conta_pagar.data.conta_pagar = utils.update_context.remover_item_pelo_id({ oldArray: state.states.pagina.conta_pagar.data.conta_pagar, itemToRemove: props._id, });
                const set_paginacao = state.states.pagina.conta_pagar.data.paginacao
                set_paginacao.itens_por_pagina = set_paginacao.itens_por_pagina - 1;
                set_paginacao.total_itens = set_paginacao.total_itens - 1;
                set_paginacao.total_itens_pagina_atual = set_paginacao.total_itens_pagina_atual - 1
    ...
    static contexto = class contexto {
        ...
        static state = class state {
            static reset() { store.setState(() => ({ states: initialStates })) }

            static set_conta_pagar_item(contaPagarData: t.Banco.Controllers.ContaPagar.BuscarPeloId.Output) {
                store.setState((state) => { state.states.modal.conta_pagar_item = contaPagarData; });
            }

            static set_conta_pagar(contaPagarData: t.Banco.Controllers.ContaPagar.BuscarPeloFiltro.Output) {
                store.setState((state) => { state.states.pagina.conta_pagar = contaPagarData; });
            }

            static async set_open_formulario(conta_pagar_id?: string) {

                store.setState((state) => { state.states.formulario.loading = true, state.states.formulario.open = true });
                if (conta_pagar_id) {
                    ...
                    } catch (error) { store.setState((state) => { state.states.formulario.loading = false; }); console.error('Erro ao buscar conta a pagar:', error); }
                }
            ...
            static set_close_formulario() {
                store.setState((state) => { state.states.formulario.loading = false, state.states.formulario.open = false });
            }

            static get_state_formulario(): ZustandStore['states']['formulario'] { return store.getState().states.formulario }

            static get_state_modal(): ZustandStore['states']['modal'] { return store.getState().states.modal }

            static get_state_pagina(): ZustandStore['states']['pagina'] { return store.getState().states.pagina }
        };
    };
